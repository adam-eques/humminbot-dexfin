...
class DexfinAPIOrderBookDataSource(OrderBookTrackerDataSource):
    ...
    async def get_last_traded_prices(cls,
        ...
        :param base_url: which Binance base_url we are connecting to (the default value is 'com')
    ...
    async def get_all_mid_prices(base_url: str = CONSTANTS.REST_URL) -> Dict[str, Decimal]:
        """
        ...
        :param base_url: base_url to use for the connection with the exchange (either "com" or "us"). Default value is "com"
        ...
        for key in resp_json:
            ...
            try:
                pair = await DexfinAPIOrderBookDataSource.trading_pair_associated_to_exchange_symbol(
                    symbol=key,
                    base_url=base_url)
                ret_val[pair] = Decimal(record['ticker'].get("avg_price", "0"))
    ...
    @classmethod
    def trading_pair_symbol_map_ready(cls, base_url: str = CONSTANTS.REST_URL):
        """
        Checks if the mapping from exchange symbols to client trading pairs has been initialized
        :param base_url: the base_url of the exchange being used (either "com" or "us"). Default value is "com"
        :return: True if the mapping has been initialized, False otherwise
        """
        return base_url in cls._trading_pair_symbol_map and len(cls._trading_pair_symbol_map[base_url]) > 0
    ...
    async def trading_pair_symbol_map(
        ...
        if not cls.trading_pair_symbol_map_ready(base_url=base_url):
        ...
        return cls._trading_pair_symbol_map[base_url]
    ...
    async def exchange_symbol_associated_to_pair(
        ...
        """
        symbol_map = await DexfinAPIOrderBookDataSource.trading_pair_symbol_map(
            base_url=base_url,
            api_factory=api_factory,
            throttler=throttler,
            time_synchronizer=time_synchronizer)
    ...
    async def fetch_trading_pairs(
        ...
        """
        mapping = await DexfinAPIOrderBookDataSource.trading_pair_symbol_map(
            base_url=base_url,
            throttler=throttler,
            api_factory=api_factory,
            time_synchronizer=time_synchronizer,
    ...
    async def listen_for_subscriptions(self):
        ...
        while True:
            try:
                ...
                await ws.connect(ws_url=CONSTANTS.WSS_URL.format(self._base_url),
    ...
    async def get_snapshot(
        ...
        data = await web_utils.api_request(
            path=CONSTANTS.SNAPSHOT_PATH_URL.format(symbol),
            api_factory=self._api_factory,
            throttler=self._throttler,
            time_synchronizer=self._time_synchronizer,
            base_url=self._base_url,
    ...
    async def _init_trading_pair_symbols(
        ...
        try:
            ...
            for symbol_data in filter(dexfin_utils.is_exchange_information_valid, data):
        ...
        cls._trading_pair_symbol_map[base_url] = mapping
