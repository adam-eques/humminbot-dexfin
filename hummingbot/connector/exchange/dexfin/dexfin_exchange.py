...
from hummingbot.connector.exchange.dexfin import dexfin_utils
...
from hummingbot.core import utils
...
class DexfinExchange(ExchangeBase):
    ...
    def __init__(self,
                 dexfin_api_key: str,
                 dexfin_secret_key: str,
                 trading_pairs: Optional[List[str]] = None,
                 trading_required: bool = True,
                 base_url: str = CONSTANTS.REST_URL,
                 ):
        self._base_url = base_url
        self._dexfin_time_synchronizer = TimeSynchronizer()
        super().__init__()
        self._trading_required = trading_required
        self._auth = DexfinAuth(
            api_key=dexfin_api_key,
            secret_key=dexfin_secret_key,
            time_provider=self._dexfin_time_synchronizer)
        self._throttler = AsyncThrottler(CONSTANTS.RATE_LIMITS)
        self._api_factory = web_utils.build_api_factory(
            throttler=self._throttler,
            time_synchronizer=self._dexfin_time_synchronizer,
            base_url=self._base_url,
            auth=self._auth)
        self._rest_assistant = None
        self._order_book_tracker = OrderBookTracker(
            data_source=DexfinAPIOrderBookDataSource(
        ...
        #     data_source=DexfinAPIUserStreamDataSource(
        #         auth=self._auth,
        #         base_url=self._base_url,
        #         throttler=self._throttler,
        #         api_factory=self._api_factory,
        ...
        self._last_trades_poll_dexfin_timestamp = 0
    ...
    @property
    def name(self) -> str:
        # if self._base_url == "com":
        return "dexfin"
        # else:
        #     return f"dexfin_{self._base_url}"
    ...
    def status_dict(self) -> Dict[str, bool]:
        ...
        """
        return {
            "symbols_mapping_initialized": DexfinAPIOrderBookDataSource.trading_pair_symbol_map_ready(
                base_url=self._base_url),
            "order_books_initialized": self._order_book_tracker.ready,
            "account_balance": len(self._account_balances) > 0 if self._trading_required else True,
            "trading_rule_initialized": len(self._trading_rules) > 0,
            # "user_stream_initialized": self._user_stream_tracker.data_source.last_recv_time > 0,
            "user_stream_initialized": "not ready",
        }
    ...
    async def cancel_all(self, timeout_seconds: float) -> List[CancellationResult]:
        ...
        try:
            ...
            self.logger().network(
                ...
                exc_info=True,
                app_warning_msg="Failed to cancel order with Dexfin. Check API key and network connection."
            )
    ...
    async def _create_order(self,
        ...
        price_str = f"{price:f}"
        type_str = DexfinExchange.dexfin_order_type(order_type)
        side_str = CONSTANTS.SIDE_BUY if trade_type is TradeType.BUY else CONSTANTS.SIDE_SELL
        symbol = await DexfinAPIOrderBookDataSource.exchange_symbol_associated_to_pair(
            trading_pair=trading_pair,
            base_url=self._base_url,
            api_factory=self._api_factory,
            throttler=self._throttler,
        ...
        try:
            ...
            self.logger().network(
                f"Error submitting {side_str} {type_str} order to Dexfin for "
        ...
        if tracked_order is not None:
            try:
                ...
                cancel_result = await self._api_request(
                    method=RESTMethod.POST,
                    path_url=CONSTANTS.ORDER_CANCEL_URL.format(exchange_id),
                    is_auth_required=True)

                # if cancel_result.get("status") == "wait" | "cancel":
                order_update: OrderUpdate = OrderUpdate(
                    client_order_id=order_id,
    ...
    async def _status_polling_loop(self):
        ...
        It also updates the time synchronizer. This is necessary because Dexfin require the time of the client to be
        ...
        while True:
            try:
                ...
                self.logger().network("Unexpected error while fetching account updates.", exc_info=True,
                                      app_warning_msg="Could not fetch account updates from Dexfin. "
        ...
        while True:
            try:
                ...
                self.logger().network("Unexpected error while fetching trading rules.", exc_info=True,
                                      app_warning_msg="Could not fetch new trading rules from Dexfin. "
    ...
    async def _format_trading_rules(self, exchange_info_dict: Dict[str, Any]) -> List[TradingRule]:
        ...
        retval = []
        for rule in filter(dexfin_utils.is_exchange_information_valid, trading_pair_rules):
            try:
                trading_pair = await DexfinAPIOrderBookDataSource.trading_pair_associated_to_exchange_symbol(
                    symbol=rule.get("id"),
                    base_url=self._base_url,
                    api_factory=self._api_factory,
                    throttler=self._throttler,
                ...
                min_notional = Decimal(rule.get("min_price"))
    ...
    async def _update_order_fills_from_trades(self):
        ...
        in case Dexfin's user stream events are not working.
        NOTE: It is not required to copy this functionality in other connectors.
        This is separated from _update_order_status which only updates the order status without producing filled
        events, since Dexfin's get order endpoint does not return trade IDs.
        ...
        if (long_interval_current_tick > long_interval_last_tick
            ...
            query_time = int(self._last_trades_poll_dexfin_timestamp * 1e3)
            self._last_trades_poll_dexfin_timestamp = self._dexfin_time_synchronizer.time()
            ...
            for trading_pair in trading_pairs:
                params = {
                    "market": await DexfinAPIOrderBookDataSource.exchange_symbol_associated_to_pair(
                        trading_pair=trading_pair,
                        base_url=self._base_url,
                        api_factory=self._api_factory,
                        throttler=self._throttler,
                ...
                for trade in trades:
                    ...
                    if exchange_order_id in order_by_exchange_id_map:
                        ...
                        fee = TradeFeeBase.new_spot_fee(
                            ...
                            flat_fees=[TokenAmount(amount=Decimal(trade["fee_amount"]), token=trade["fee_currency"])]
                        )
                        trade_update = TradeUpdate(
                        ...
                        result = await self._api_request(RESTMethod.GET,
                                                         path_url=CONSTANTS.ORDER_INFO_URL.format(exchange_id),
                                                         params={},
                                                         is_auth_required=True)
    ...
    async def _update_order_status(self):
        # This is intended to be a backup measure to close straggler orders, in case Dexfin's user stream events
        # are not working.
        # The minimum poll interval for order status is 10 seconds.
        ...
        if current_tick > last_tick and len(tracked_orders) > 0:
            ...
            for order_update, tracked_order in zip(results, tracked_orders):
                ...
                if isinstance(order_update, Exception):
                    ...
                    new_state = CONSTANTS.ORDER_STATE[str(order_update["state"])]
    ...
    async def _update_time_synchronizer(self):
        try:
            await self._dexfin_time_synchronizer.update_server_time_offset_with_time_provider(
            ...
            self.logger().exception("Error requesting time from Dexfin server")
            raise
