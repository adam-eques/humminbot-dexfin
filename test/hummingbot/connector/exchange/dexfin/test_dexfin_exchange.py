...
class DexfinExchangeTests(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.exchange = DexfinExchange(
            dexfin_api_key="testAPIKey",
            dexfin_secret_key="testSecret",
    ...
    def test_cancel_order_successfully(self, mock_api):
        ...
        url = web_utils.private_rest_url(CONSTANTS.ORDER_CANCEL_URL.format("([0-9]*)"))
        ...
        #     "remaining_volume": str(order.amount - Decimal("0")),
        #     "side": "buy",
        #     "state": "wait",
        #     "trades_count": 6,
        #     "updated_at": "2020-03-12T17:01:56+01:00"
        ...
        # self.assertEqual(self.exchange_trading_pair, request_params["market"])
    ...
    def test_update_time_synchronizer_successfully(self, mock_api, seconds_counter_mock):
        ...
        self.exchange._dexfin_time_synchronizer.clear_time_offset_ms_samples()
        url = web_utils.private_rest_url(CONSTANTS.SERVER_TIME_PATH_URL)
        regex_url = re.compile(f"^{url}".replace(".", r"\.").replace("?", r"\?"))

        response = "2022-06-29T18:33:41+00:00"
    ...
    def test_update_balances(self, mock_api):
        url = web_utils.public_rest_url(CONSTANTS.SERVER_TIME_PATH_URL, base_url=self.exchange._base_url)
        regex_url = re.compile(f"^{url}".replace(".", r"\.").replace("?", r"\?"))
        ...
        response = [
            ...
            {
                "currency": "vr",
                "balance": "32749.184025",
                "locked": "98247.552075"
            },
            {
                "currency": "czk",
                "balance": "6423.427783472394",
                "locked": "0.0"
            }
        ]
        ...
        pprint(available_balances)
        pprint(total_balances)

        self.assertEqual(Decimal("35627.771188101"), available_balances["usdt"])
        self.assertEqual(Decimal("32749.184025"), available_balances["vr"])
        self.assertEqual(Decimal("35627.771188101"), total_balances["usdt"])
        self.assertEqual(Decimal("130996.736100"), total_balances["vr"])
        ...
        self.assertNotIn("pnk", available_balances)
        self.assertNotIn("bnb", total_balances)
        self.assertEqual(Decimal("32749.184025"), available_balances["vr"])
        self.assertEqual(Decimal("130996.736100"), total_balances["vr"])
    ...
    def test_update_order_fills_from_trades_triggers_filled_event(self, mock_api):
        ...
        trade_fill = {
            ...
            "fee": "0.1",
            "fee_amount": "1.061141317",
            "market": self.exchange_trading_pair,
            "created_at": "2021-05-04T02:07:27Z",
            "taker_type": "sell",
            "side": "sell",
        ...
        url = web_utils.private_rest_url(CONSTANTS.ORDER_INFO_URL.format("([0-9]*)"))
        regex_url = re.compile(f"^{url}")
        mock_api.get(regex_url, body=json.dumps(self.get_order_state_mock(
            exchange_order_id="8625610"
        )))
        ...
        pprint(mock_api.requests.items())
        ...
        self.assertEqual([TokenAmount(trade_fill["fee_currency"], Decimal(trade_fill["fee_amount"]))],
        ...
        self.assertEqual([
            TokenAmount(
                trade_fill_non_tracked_order["fee_currency"],
                Decimal(trade_fill_non_tracked_order["fee_amount"]))],
            fill_event.trade_fee.flat_fees)
    ...
    def test_update_order_fills_request_parameters(self, mock_api):
        ...
        self.exchange._last_poll_timestamp = (self.exchange.current_timestamp -
                                              self.exchange.UPDATE_ORDER_STATUS_MIN_INTERVAL - 1)
        self.exchange._last_trades_poll_dexfin_timestamp = 10
        self.async_run_with_timeout(self.exchange._update_order_fills_from_trades())

        trades_request = next((key, value) for key, value in mock_api.requests.items())
        request_params = trades_request[1][0].kwargs["params"]
        self.assertEqual(self.exchange_trading_pair, request_params["market"])
        # self.assertEqual(10 * 1e3, request_params["timefrom"])
        self._validate_auth_credentials_for_request(trades_request[1][0])
    ...
    def test_update_order_fills_from_trades_with_repeated_fill_triggers_only_one_event(self, mock_api):
        ...
        self.assertEqual(0.0, fill_event.trade_fee.percent)
        self.assertEqual([
            TokenAmount(trade_fill_non_tracked_order["fee_currency"],
                        Decimal(trade_fill_non_tracked_order["fee_amount"]))],
            fill_event.trade_fee.flat_fees)
        self.assertTrue(self._is_logged(
            "INFO",
    ...
    def test_update_order_status_when_filled(self, mock_api):
        ...
        print(self.buy_order_completed_logger.event_log)
        print(self.buy_order_created_logger.event_log)
        print(self.order_cancelled_logger.event_log)
        print(self.order_failure_logger.event_log)
        print(self.order_filled_logger.event_log)
        print(self.sell_order_completed_logger.event_log)
        print(self.sell_order_created_logger.event_log)
    ...
    def test_update_trading_rules(self, mock_api):
        ...
        self.assertEqual(Decimal(status["min_amount"]),
                         trading_rule.min_order_size)
        self.assertEqual(Decimal(f"1e-{status['price_precision']}"),
                         trading_rule.min_price_increment)
        self.assertEqual(Decimal(f"1e-{status['amount_precision']}"),
                         trading_rule.min_base_amount_increment)
        self.assertEqual(Decimal(status.get("min_price")),
    ...
    def test_update_trading_rules_ignores_rule_with_error(self, mock_api):
        ...
        self.assertTrue(
            self._is_logged("ERROR", f"Error parsing the trading pair rule {order_status[0]}. Skipping.")
        )
